...
contract('StakingRewards', accounts => {
	...
	before(async () => {
		...
		stakingRewards = await setupContract({
			...
			args: [owner, rewardsDistribution.address, rewardsToken.address, stakingToken.address, toUnit('10')],
		});
	...
	describe('stake()', () => {
		...
		it('cannot stake the amount less than minStakeAmount', async () => {
			await assert.revert(stakingRewards.stake('9'), 'Stake amount is less than limit.');
		});
	});
	...
	describe('getReward()', () => {
		...
		it('Should not get reward before a set of time from staked', async () => {
			const totalToStake = toUnit('100');
			const totalToDistribute = toUnit('5000');
			...
			assert.revert(stakingRewards.getReward({ from: stakingAccount1 }), 'Can not withdraw or get reward');
		})
	});
	...
	describe('setRewardsDuration()', () => {
		...
		it('should update when setting setRewardsDuration after the period has finished', async () => {
			...
			assert.require(stakingRewards.getReward({ from: stakingAccount1 }), 'Can not withdraw or get reward');
			await fastForward(DAY * 4);
	...
	describe('withdraw()', () => {
		...
		it('should not withdraw before a set of time from staked', async () => {
			const totalToStake = toUnit('100');
			await stakingToken.transfer(stakingAccount1, totalToStake, { from: owner });
			await stakingToken.approve(stakingRewards.address, totalToStake, { from: stakingAccount1 });
			await stakingRewards.stake(totalToStake, { from: stakingAccount1 });

			await assert.revert(stakingRewards.withdraw(totalToStake, { from: stakingAccount1 }), 'Can not withdraw or get reward');
		});
